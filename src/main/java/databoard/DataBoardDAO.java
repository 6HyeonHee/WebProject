...
public class DataBoardDAO extends DBConnPool2 {
	...
	public List<DataBoardDTO> selectListPage(Map<String, Object> map) {
		...
		// databoard 테이블을 대상으로 하므로 타입 매개변수 확인 필요함
		List<DataBoardDTO> databoardLists = new Vector<DataBoardDTO>();
	...
	// 매개변수로 전달된 게시물의 일련번호로 게시물을 인출한다.
		public DataBoardDTO selectView(String num) {
			...
			String query = "SELECT B.*, M.name "
					+ " FROM member M INNER JOIN databoard B "
					+ " ON M.id=B.id "
					+ " WHERE idx=? ";
